...
python Main.py -i "ML(Dataset('errors_at_end', 3), use_multiprocessing=False)" -o "batch_size" "[2**n for n in range(7)]"
...
python Main.py -i "ML(Dataset('errors_at_end', 3), use_multiprocessing=False)" -o "repeats" "[3, 4, 5]"

python Main.py -i "ML(Dataset('errors_at_end', 3), use_multiprocessing=False)" -o "noise_variation" "[round(0.1 * n, 1) for n in range(11)]"
python Main.py -i "ML(Dataset('errors_at_end', 3), use_multiprocessing=False)" -o "amplitude_variation" "[round(0.1 * n, 1) for n in range(11)]"
python Main.py -i "ML(Dataset('errors_at_end', 3), use_multiprocessing=False)" -o "phase_variation" "[round(0.2 * n, 1) for n in range(11)]"