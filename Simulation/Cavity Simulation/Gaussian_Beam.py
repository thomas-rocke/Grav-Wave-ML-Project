...
class Hermite:
    ...
    def __init__(self, l: int = -1, m: int = -1, amplitude: float = 1.0, phase:float=0, w_0: float = 0.4, wavelength: float = 600.0, n: int = 500):
        '''
        Initialise class.
        '''
        self.l = l
...
#        return np.array((w_ratio * exp_1 * exp_2) * self.amplitude * np.e**(1j*self.phase))
    ...
    def E_mode(self, x, y, z):
        ...
        '''
        return np.array(self.amplitude * np.e**(1j*self.phase) * (self.u(x, z, self.l) * self.u(y, z, self.m) * np.exp(-1j * self.k * z)))
...
class Superposition(list):
    ...
    def __init__(self, modes: list, amplitude: float = 1.0, phase: float=0):
        '''
...
class Generate_Data(list):
    ...
    def __init__(self, max_order: int = 1, number_of_modes: int = 1, amplitude_variation: float = 0.0, phase_variation:float = 0.0, repeats: int = 1, info: bool = True):
        '''
