...
class Hermite:
    ...
    def __init__(self, l: int = -1, m: int = -1, amplitude: float = 1.0, phase: float = 0.0, w_0: float = 0.4, wavelength: float = 1064.0, n: int = 500, resolution = 128, update_field:bool=True):
        '''
        Initialise class.
        '''
        self.l = l
        ...
        if update_field:
            ...
            self._E_mode = self.E_mode(X, Y, 0)
    ...
    def copy(self):
        ...
        '''
        mode = Hermite(self.l, self.m, self.amplitude, self.phase, self.w_0, self.wavelength, self.n, self.resolution, update_field=False)
        mode._E_mode = self._E_mode # Raw Electric fields should be identical
        return mode
...
#        return np.array((w_ratio * exp_1 * exp_2) * self.amplitude * np.e**(1j*self.phase))
    def E_mode(self, x, y, z):
        ...
        '''
        return np.array(self.u(x, z, self.l) * self.u(y, z, self.m) * np.exp(-1j * self.k * z))
...
class Superposition(list):
    ...
    def contains(self, mode: Hermite):
        ...
        return Hermite(l=0, m=0, amplitude=0, phase=-10, update_field=False)
    ...
    def superpose(self):
        ...
        '''
        facts = np.fromiter((mode.amplitude * np.exp(mode.phase * 1j) for mode in self), complex, count=len(self))
        grids = np.array([mode._E_mode for mode in self])

        superposition = np.abs(np.tensordot(facts, grids, axes=1)**2)
    ...
    def phase_map(self):
        ...
        facts = np.fromiter((mode.amplitude * np.exp(mode.phase * 1j) for mode in self), complex, count=len(self))
        grids = np.array([mode._E_mode for mode in self])
        superposition = np.tensordot(facts, grids, axes=1)**2
...
class Laguerre(Superposition):
    ...
    def E_mode(self, x, y, z):
        ...
        e_field = np.tensordot(facts, grids, axes=1)

        return e_field # Normalise the superposition
