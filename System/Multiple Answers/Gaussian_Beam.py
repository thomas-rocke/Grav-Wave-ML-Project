...
class Generate_Data(list):
    ...
    def get_inputs(self, desc: str = None):
        '''
        Get all the superpositions in the dataset.
        '''
        superpositions = self.superpose(desc)
        tmp = []
        for i in range(len(superpositions)): [tmp.append(superpositions[i]) for j in range(len(self[i]))]
        return np.array(tmp)

    def get_outputs(self):
        '''
        Get all possible Gaussian modes that could comprise a superposition.
        Output: [all possible solutions to the network (no. of classes), index for each input to its output].
        '''
        tmp = []
        for i in self.combs:
            for j in i:
                for k in range(len(self.gauss_modes)):
                    if str(self.gauss_modes[k]) == str(j): tmp.append([k])
            # [tmp.append([j]) for j in range(len(i))]
        return np.array(self.repeats * tmp)
